...
{
	[AddComponentMenu("Koreographer/Demos/Rhythm Game/Position Controller")]
	public class PositionNotesControl : MonoBehaviour
	{
		#region Fields
		...
		public void Initialize(LevelDisplay controller)
		{
		...
		int GetSpawnSampleOffset()
		{
			...
			double spawnSecsToTarget = (double)spawnDistToTarget / (double)_gameController.noteSpeed;

			// Figure out the samples to the target.
			return (int)(spawnSecsToTarget * _gameController.SampleRate);
		}
		...
		public void CheckNoteHit()
		{
			...
			{
				NotePrefab hitNote = trackedNotes.Dequeue();
